...
if( A_checkpermission(array(0,2,0,4,0)) ) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if(isset($_POST['edit_station'])) {
            ...
            //  edit station data
            S_set_data($Db,'UPDATE Station SET Ort=\''.$station_ort.'\',Adresse=\''.$address.'\',Oeffnungszeiten=\''.$opening.'\' WHERE id='.$user_id.';');
            ...
            if($station_ort=='' || $new_id>0) {
                ...
                S_set_data($Db,'INSERT INTO Station (id, Ort, Adresse) VALUES (
    ...
    // Get user details
    $array_staff=S_get_multientry($Db,'SELECT id, Ort, Adresse FROM Station;');
    ...
    $array_station=S_get_multientry($Db,'SELECT Station.id, Station.Ort, Station.Adresse, Station.Oeffnungszeiten FROM Station ORDER BY Station.id ASC;');
