...
if( A_checkpermission(array(1,2,0,4,5)) ) {
    ...
    // Mit Terminbuchung
    $array_termine_reservation=S_get_multientry($Db,'SELECT id,Tag,Stunde,Slot,0,0,count(Slot),sum(Used) FROM Termine WHERE Slot>0 AND id_station='.$station.' AND Date(Tag)>=\''.$today.'\' GROUP BY id_station,Tag,Stunde,Slot;');
    ...
    if( A_checkpermission(array(0,2,0,4,0)) ) {
        echo '<div class="col-sm-12">
        <div class="card">
        <h3>Alle Stationen in den nächsten Tagen</h3>';
