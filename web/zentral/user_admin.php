    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if(isset($_POST['edit_staff'])) {
            ...
            if($old_username==$username || !(S_get_entry($Db,'SELECT id FROM li_user WHERE username=\''.$username.'\';')>0) ) {
                ...
                //  edit staff data
                S_set_data($Db,'UPDATE li_user SET username=\''.$username.'\', login_attempts=CAST('.$attempts.' AS int), Station='.$station_id.', role_1='.$u_role_1.', role_2='.$u_role_2.', role_3='.$u_role_3.', role_4='.$u_role_4.', role_5='.$u_role_5.'  WHERE id=CAST('.$user_id.' AS int);');
